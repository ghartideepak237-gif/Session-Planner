...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) return; // Don't interfere with horizontal scroll
      e.preventDefault();
      const now = Date.now();
      ...
      const ease = () => {
        ...
        velocity *= 0.88; // inertia multiplier
  ...
  return (
    <div className={`page-wrapper ${isMobile ? 'is-mobile' : ''}`}>
      ...
      <nav className={`navbar glass-header ${scrolled ? 'scrolled' : ''}`}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }} onClick={() => setActiveTab('repository')}>
          {isMobile && <Menu onClick={(e) => { e.stopPropagation(); setIsMenuOpen(true); }} color="#FFF" style={{ marginRight: '8px' }} />}
          <div style={{ background: 'rgba(255,255,255,0.05)', padding: '8px', borderRadius: '10px' }}><img src="/e-logo.png" style={{ height: '24px' }} alt="logo" /></div>
          <span style={{ color: '#FFF', fontWeight: '800', fontSize: '18px' }}>Session Planner</span>
        </div>
        {!isMobile && (
        ...
        <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#FFF', padding: '10px 20px', borderRadius: '12px', cursor: 'pointer' }}>Logout</button>
      </nav>
      ...
      {/* Global FABs */}
      <AnimatePresence>
        <div style={{ position: 'fixed', bottom: 40, right: 40, zIndex: 10000, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {activeTab === 'repository' && (
            <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} onClick={() => setShowAddGame(true)} style={{ width: '54px', height: '54px', borderRadius: '50%', background: 'var(--accent-gold)', border: 'none', color: '#000', cursor: 'pointer', boxShadow: '0 10px 20px rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Plus size={24} strokeWidth={3} />
            </motion.button>
          )}
          {scrolled && (
            <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} onClick={() => scrollRef.current?.scrollTo({ top: 0, behavior: 'smooth' })} style={{ width: '54px', height: '54px', borderRadius: '50%', background: 'rgba(15, 19, 24, 0.8)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.1)', color: '#FFF', cursor: 'pointer', boxShadow: '0 10px 20px rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ArrowUp size={24} />
            </motion.button>
          )}
        </div>
